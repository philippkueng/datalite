          ...
          (condp = operation
            ...
            ::xt/put (let [table-name (->> (keys entry)
                       ...
                       (jdbc/with-db-transaction [t-conn connection]
                         ...
                         ;; Check if there's a entry in the database already for the provided :xt/id & valid_at time
                         ...
                         (let [_insertion (jdbc/insert! t-conn
                                            ...
                                            (assoc (reduce (fn [non-namespaced-entry namespaced-key]
                                              ...
                                              :valid_from (Timestamp/from valid-time)))
